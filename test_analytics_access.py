...
def test_analytics_access():
    ...
    print(f"Analytics location: {analytics_response.headers.get('Location', 'No redirect')}")

    # Try admin_analytics
    print("\n5. Testing admin_analytics access after login...")
    admin_analytics_response = session.get(f"{base_url}/admin_analytics", allow_redirects=False)
    ...
    if admin_analytics_response.status_code == 200:
        ...
        print(f"✗ Admin analytics failed with status {admin_analytics_response.status_code}")
