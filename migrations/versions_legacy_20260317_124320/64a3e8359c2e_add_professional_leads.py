...
def upgrade():
    ...
    with op.batch_alter_table("professional_leads", schema=None) as batch_op:
        if batch_op.f("ix_professional_leads_city") not in indexes:
            safe_create_index(op, 'professional_leads', batch_op.f("ix_professional_leads_city"), ["city"], unique=False)
        if batch_op.f("ix_professional_leads_email") not in indexes:
            safe_create_index(op, 'professional_leads', batch_op.f("ix_professional_leads_email"), ["email"], unique=False)
...
def downgrade():
    ...
    with op.batch_alter_table("professional_leads", schema=None) as batch_op:
        if batch_op.f("ix_professional_leads_profession") in indexes:
            safe_drop_index(op, batch_op.f("ix_professional_leads_profession"))
        if batch_op.f("ix_professional_leads_email") in indexes:
            safe_drop_index(op, batch_op.f("ix_professional_leads_email"))
        if batch_op.f("ix_professional_leads_city") in indexes:
            safe_drop_index(op, batch_op.f("ix_professional_leads_city"))
