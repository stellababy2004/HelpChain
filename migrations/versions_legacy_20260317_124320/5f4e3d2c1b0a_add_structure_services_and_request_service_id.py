...
def upgrade():
    ...
    if not index_exists(bind, "requests", "ix_requests_service_id"):
        safe_create_index(op, "ix_requests_service_id", "requests", ["service_id"], unique=False)
...
def downgrade():
    ...
    if table_exists(bind, "requests") and column_exists(bind, "requests", "service_id"):
        if bind.dialect.name != "sqlite" and fk_exists(bind, "requests", "fk_requests_service_id"):
            safe_drop_constraint(op, "fk_requests_service_id", "requests", type_="foreignkey")

        if index_exists(bind, "requests", "ix_requests_service_id"):
            safe_drop_index(op, "ix_requests_service_id", table_name="requests")

        safe_drop_column(op, "requests", "service_id")
