...
def downgrade():
    ...
    if dialect == "sqlite":
        with op.batch_alter_table("requests", recreate="always") as batch_op:
            try:
                safe_drop_constraint(op, fk_name, type_="foreignkey", table_name='requests')
            except Exception:
                pass
            safe_drop_column(op, 'requests', "structure_id")
    else:
        if fk_name in _fk_names(insp, "requests"):
            safe_drop_constraint(op, fk_name, "requests", type_="foreignkey")
        safe_drop_column(op, "requests", "structure_id")