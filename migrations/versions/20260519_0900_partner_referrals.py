...
def upgrade():
    ...
    if not _has_table(bind, "case_referrals"):
        op.create_table(
            ...
            sa.Column("operational_status", sa.String(length=32), server_default="sent", nullable=False),
            ...
            sa.Column("last_public_update_at", sa.DateTime(timezone=True), nullable=True),
            sa.Column("public_status_note", sa.Text(), nullable=True),
            sa.Column("created_at", sa.DateTime(timezone=True), nullable=False),
    ...
    for index_name, columns, unique in (
        ...
        ("ix_case_referrals_operational_status", ["operational_status"], False),
