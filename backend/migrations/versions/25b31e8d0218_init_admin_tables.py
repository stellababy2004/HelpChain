...
Revision ID: 25b31e8d0218
Revises: 87b6c47a927d
Create Date: 2025-11-01 14:04:51.859068
...
revision = '25b31e8d0218'
down_revision = '87b6c47a927d'
branch_labels = None
depends_on = None
...
def upgrade():
    ...
    with op.batch_alter_table('failed_emails', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_failed_emails_recipient'), ['recipient'], unique=False)
    ...
    sa.Column('is_active', sa.Boolean(), nullable=True),
    sa.Column('priority', sa.Enum('low', 'normal', 'urgent', name='priorityenum'), nullable=True),
    ...
    sa.Column('role', sa.Enum('user', 'volunteer', 'moderator', 'admin', 'superadmin', name='roleenum'), nullable=False),
    ...
    with op.batch_alter_table('volunteers', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_volunteers_location'), ['location'], unique=False)
    ...
    sa.Column('priority', sa.Enum('low', 'normal', 'urgent', name='priorityenum'), nullable=False),
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.Column('user_id', sa.Integer(), nullable=True),
    sa.Column('name', sa.String(length=100), nullable=False),
    ...
    with op.batch_alter_table('help_requests', schema=None) as batch_op:
        batch_op.create_index(batch_op.f('ix_help_requests_assigned_volunteer_id'), ['assigned_volunteer_id'], unique=False)
        batch_op.create_index(batch_op.f('ix_help_requests_city'), ['city'], unique=False)
        batch_op.create_index(batch_op.f('ix_help_requests_completed_at'), ['completed_at'], unique=False)
        batch_op.create_index(batch_op.f('ix_help_requests_location_text'), ['location_text'], unique=False)
        batch_op.create_index(batch_op.f('ix_help_requests_region'), ['region'], unique=False)
    ...
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['assigned_to'], ['volunteers.id'], ),
    ...
    sa.Column('session_id', sa.String(length=128), nullable=False),
    sa.Column('user_type', sa.String(length=20), nullable=True),
    sa.Column('activity_type', sa.Enum('PAGE_VIEW', 'PAGE_EXIT', 'SCROLL', 'TIME_SPENT', 'BUTTON_CLICK', 'FORM_SUBMIT', 'FORM_START', 'LINK_CLICK', 'SEARCH_QUERY', 'HELP_REQUEST_CREATED', 'HELP_REQUEST_VIEWED', 'HELP_REQUEST_UPDATED', 'TASK_VIEWED', 'TASK_ACCEPTED', 'TASK_COMPLETED', 'VOLUNTEER_PROFILE_VIEWED', 'CHAT_MESSAGE_SENT', 'CHAT_ROOM_JOINED', 'VIDEO_CHAT_STARTED', 'LOGIN', 'LOGOUT', 'PASSWORD_RESET', 'ACHIEVEMENT_UNLOCKED', 'POINTS_EARNED', 'LEVEL_UP', 'ERROR_OCCURRED', 'PAGE_NOT_FOUND', 'FORM_VALIDATION_ERROR', 'REGISTRATION_COMPLETED', 'HELP_REQUEST_ASSIGNED', 'TASK_ASSIGNED', name='useractivitytypeenum'), nullable=False),
    ...
    with op.batch_alter_table('user_activities', schema=None) as batch_op:
        ...
        batch_op.create_index('idx_user_activities_related', ['related_type', 'related_id'], unique=False)
        batch_op.create_index('idx_user_activities_type_timestamp', ['activity_type', 'timestamp'], unique=False)
        batch_op.create_index('idx_user_activities_user_session', ['user_id', 'session_id'], unique=False)
    ...
    sa.Column('title', sa.String(length=200), nullable=False),
    sa.Column('message', sa.Text(), nullable=False),
    sa.Column('notification_type', sa.Enum('system', 'request', 'task', 'message', 'achievement', 'reminder', 'alert', name='notificationtypeenum'), nullable=False),
    sa.Column('recipient_id', sa.Integer(), nullable=False),
    sa.Column('recipient_type', sa.String(length=20), nullable=False),
    sa.Column('channels', sa.JSON(), nullable=False),
    sa.Column('email_status', sa.Enum('pending', 'sent', 'delivered', 'read', 'failed', 'cancelled', name='notificationstatusenum'), nullable=True),
    sa.Column('app_status', sa.Enum('pending', 'sent', 'delivered', 'read', 'failed', 'cancelled', name='notificationstatusenum'), nullable=True),
    sa.Column('push_status', sa.Enum('pending', 'sent', 'delivered', 'read', 'failed', 'cancelled', name='notificationstatusenum'), nullable=True),
    sa.Column('priority', sa.Enum('low', 'normal', 'urgent', name='priorityenum'), nullable=False),
    ...
    sa.Column('created_at', sa.DateTime(), nullable=True),
    sa.Column('updated_at', sa.DateTime(), nullable=True),
    sa.ForeignKeyConstraint(['queue_id'], ['notification_queue.id'], ),
    sa.ForeignKeyConstraint(['template_id'], ['notification_templates.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    with op.batch_alter_table('notifications', schema=None) as batch_op:
        batch_op.create_index('idx_notifications_created', ['created_at'], unique=False)
        batch_op.create_index('idx_notifications_recipient', ['recipient_id', 'recipient_type'], unique=False)
        batch_op.create_index('idx_notifications_status', ['email_status', 'app_status', 'push_status'], unique=False)
