...
def upgrade():
    ...
    # Guard: avoid creating PostgreSQL native ENUM types when running against
    # SQLite (used in local dev/tests). When not on Postgres, force SQLAlchemy
    # to render enums as non-native (VARCHAR + CHECK) by setting
    # native_enum=False via a local replacement of sa.Enum. This prevents
    # Alembic/SQLAlchemy from issuing `CREATE TYPE ... AS ENUM` statements
    # against SQLite which will fail.
    bind = op.get_bind()
