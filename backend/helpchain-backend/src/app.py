...
Умения: {volunteer.skills}"""
    ...
    from models import db, ChatRoom, ChatMessage, User
    from analytics_service import analytics_service
    import os
    ...
    def create_app(config_object):
        ...
        with app.app_context():
            ...
            if User.query.count() == 0:
                ...
                db.session.commit()
                print(
                    "Sample user created: email=test@example.com, password=password123"
                )
                print("Admin user created: email=admin@example.com, password=admin123")
...
Умения: {volunteer.skills}"""
            ...
            try:
                ...
                print(
                    f"✅ Notification saved to database for volunteer ID {volunteer.id}"
                )
            except Exception as e:
                print(f"❌ Failed to save to database: {e}")
        ...
        def dashboard():
            ...
            if current_user.is_admin:
                ...
                requests = (
                    Request.query.all()
                )  # Changed to show all requests for non-admin
            return render_template("dashboard.html", requests=requests)
