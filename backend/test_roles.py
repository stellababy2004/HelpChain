...
def test_admin_roles_dashboard():
    ...
    assert response2.status_code == 200, f"Роли dashboard недостъпен: {response2.status_code}"
    content = response2.text
    assert "Роли" in content and "Права" in content, "Съдържанието не съдържа роли и права"
