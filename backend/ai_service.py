...
def _sanitize_api_key(raw_key: Any) -> Tuple[str, list]:
    ...
    placeholder_indicators = [
        'ваш', 'your', 'ключ', 'key', 'placeholder', 'tua', 'tuo', 'вашия', 'ваш_к', 'sk-ваш'
    ]
...
class AIService:
    ...
    async def generate_response(self, user_message: str, context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
    ...
    def _build_context(self, user_message: str, context: Optional[Dict[str, Any]], language: str) -> str:
    ...
    def test_connection(self) -> Dict[str, Any]:
        ...
        for name, provider in ai_config.providers.items():
            if mock_flag:
                results[name] = {'status': 'ok', 'message': 'Mock mode enabled (AI_DEV_MOCK)'}
                continue

            if not provider.enabled:
                results[name] = {'status': 'disabled', 'message': 'API key not provided'}
                continue
            ...
            try:
                if name == 'openai':
                    ...
                    if openai is None:
                        results[name] = {'status': 'disabled', 'message': 'openai package not installed'}
                    else:
                        ...
                        try:
                            ...
                            if issues:
                                results[name] = {'status': 'error', 'message': '; '.join(issues)}
                                continue
                            ...
                            if resp.status_code == 200:
                                ...
                                results[name] = {'status': 'error', 'message': 'Incorrect API key provided'}
                            else:
                                results[name] = {'status': 'error', 'message': f'HTTP {resp.status_code}: {resp.text[:300]}'}
                        except Exception as e:
                                ...
                                try:
                                    ...
                                    try:
                                        ...
                                        if hasattr(openai, 'ChatCompletion'):
                                            ...
                                            openai.ChatCompletion.create(model='gpt-3.5-turbo', messages=[{'role':'user','content':'Test'}], max_tokens=1)
                                            results[name] = {'status': 'ok', 'message': 'Connection successful (via SDK fallback)'}
                                            continue
                                        else:
                                            client = openai.OpenAI()
                                            client.chat.completions.create(model='gpt-3.5-turbo', messages=[{'role':'user','content':'Test'}], max_tokens=1)
                                            results[name] = {'status': 'ok', 'message': 'Connection successful (via SDK fallback)'}
                                            continue
                                    except Exception as sdk_e:
                                        results[name] = {'status': 'error', 'message': _safe_message(sdk_e)}
                                        continue
                                except Exception:
                    ...
                    # Test Gemini
                    if genai is None:
                        results[name] = {'status': 'disabled', 'message': 'google.generativeai package not installed'}
                    else:
