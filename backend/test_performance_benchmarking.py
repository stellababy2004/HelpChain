...
class TestPerformanceBenchmarking:
    ...
    def test_api_response_times(self, client, authenticated_volunteer_client, authenticated_admin_client):
        ...
        for endpoint, test_client in endpoints:
            ...
            if response_times:
                ...
                assert results[endpoint]['avg_response_time'] < 2000, f"{endpoint} too slow: {results[endpoint]['avg_response_time']}ms"
    ...
    def test_database_query_performance(self, db_session):
        ...
        start_time = time.time()
        recent_requests = db_session.query(HelpRequest).filter(
            HelpRequest.created_at >= datetime.utcnow() - timedelta(days=30)
        ).limit(100).all()
        end_time = time.time()
    ...
    def test_concurrent_requests_simulation(self, client, authenticated_volunteer_client):
        ...
        # Analyze results
        successful_requests = [r for r in all_results if 'status_code' in r and r['status_code'] in [200, 404]]
        ...
        if response_times:
            ...
            assert avg_response_time < 3000, f"Average response time too slow under concurrency: {avg_response_time}ms"
            ...
            print(f"Concurrent Performance - Total time: {total_time:.2f}ms, "
    ...
    def test_cache_performance(self, client, authenticated_volunteer_client):
        ...
        if response1.status_code == response2.status_code == 200:
            ...
            improvement_percentage = (improvement / first_request_time) * 100 if first_request_time > 0 else 0

            # Log cache performance (don't assert as caching may not be implemented for all endpoints)
            print(f"Cache Performance - First: {first_request_time:.2f}ms, "
    ...
    def test_database_connection_pooling(self, db_session):
        ...
        assert avg_connection_time < 50, f"Database connections too slow: {avg_connection_time}ms"

        print(f"Database Connection Performance - Avg time: {avg_connection_time:.2f}ms")
    ...
    def test_static_asset_performance(self, client):
        ...
        for asset_url in asset_endpoints:
            ...
            if response.status_code == 200:
                ...
                assert load_time < 500, f"Asset {asset_url} loads too slowly: {load_time}ms"
    ...
    def test_error_handling_performance(self, client):
        ...
        for endpoint in error_endpoints:
            ...
            assert error_time < 1000, f"Error handling too slow for {endpoint}: {error_time}ms"
    ...
    def test_json_serialization_performance(self, authenticated_volunteer_client):
        ...
        if response.status_code == 200:
            ...
            print(f"JSON Serialization Performance - Parse time: {avg_parse_time:.2f}ms")
