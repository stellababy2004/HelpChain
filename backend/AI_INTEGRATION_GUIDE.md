...
Допълнителна автоматична екстракция (диагностична и безопасна):

- Внимание: автоматичното извличане и използване на API ключове е рисковано. Проектът може да опита да открие първия ASCII-подобен низ от вида `sk-[A-Za-z0-9_-]{20,100}` само в локални диагностични контексти и за кратки health checks.
- Тази функция е изключена по подразбиране в production. Никога не използваме автоматично извлечен низ за реални заявки към доставчици без явна администраторска експлицитна потвърждение и допълнителни проверки.
- Всички опити за автоматична екстракция се логват и маркират като диагностични; ако бъде открит възможен ключ, трябва да го завъртите/ревокирате и да следвате стандартния процес за ротация на ключове.
