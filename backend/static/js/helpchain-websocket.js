...
class HelpChainWebSocket {
  constructor(serverUrl) {
    ...
    this.defaultTransports = this.forcePolling
      ? ["polling"]
      : ["websocket", "polling"];
    this.lastUserInfo = {};
  ...
  getConfiguredTransports() {
    ...
    if (dataset?.socketTransports) {
      console.debug(
        "Detected socket transports from dataset:",
        dataset.socketTransports,
      );
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        const supportsWebSocket =
          !this.forcePolling && transports.includes("websocket");
        console.info(
          "HelpChain socket transports:",
          transports,
          "upgrade:",
          supportsWebSocket,
        );
        ...
        this.socket.on("connect_error", (error) => {
          if (
            supportsWebSocket &&
            !this.hasDowngradedToPolling &&
            this.isInvalidTransportError(error)
          ) {
            console.warn(
              "Server rejected websocket transport; forcing polling-only mode.",
            );
            this.forcePolling = true;
            this.hasDowngradedToPolling = true;
            window.HELPCHAIN_FORCE_POLLING = true;
            ...
            setTimeout(() => {
              this.connect(this.lastUserInfo).then(resolve).catch(reject);
            }, this.reconnectDelay);
            return;
          }
          ...
          if (supportsWebSocket && !this.hasDowngradedToPolling) {
            ...
            console.info(
              "Socket connection issue (polling mode):",
              error?.message || error,
            );
          }
