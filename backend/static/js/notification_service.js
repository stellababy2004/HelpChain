...
class HelpChainNotificationService {
    ...
    async subscribeToPush() {
        ...
        try {
            const response = await fetch('/api/notification/subscribe', {
                ...
                body: JSON.stringify({
                    endpoint: this.swRegistration.scope,
    ...
    showNewRequestNotification(request) {
        if (this.pushPermission !== 'granted' || !this.settings.pushEnabled || !this.settings.notifyNewRequests) {
            return;
        }
    ...
    showUrgentNotification(request) {
        if (this.pushPermission !== 'granted' || !this.settings.pushEnabled || !this.settings.notifyUrgentRequests) {
            return;
        }
    ...
    showMessageNotification(message) {
        if (this.pushPermission !== 'granted' || !this.settings.pushEnabled || !this.settings.notifyMessages) {
            return;
        }

        const notification = new Notification(`Ново съобщение от ${message.sender}`, {
    ...
    showTaskUpdateNotification(task) {
        if (this.pushPermission !== 'granted' || !this.settings.pushEnabled || !this.settings.notifyTaskUpdates) {
            return;
        }
    ...
    async saveSettings(settings) {
        try {
            ...
            if (response.ok) {
                ...
                localStorage.setItem('helpchain_notifications', JSON.stringify(settings));
    ...
    updateUI() {
        ...
        const checkboxes = [
            'notifyNewRequests', 'notifyUrgentRequests', 'notifyMessages',
            'notifyTaskUpdates', 'notifyVolunteerFound', 'notifyTaskProgress',
            'notifyCompletion', 'notifyFeedback'
        ];
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => notificationService.initialize());
} else {
