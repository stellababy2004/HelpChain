...
class LoadTester:
    ...
    def run_load_test(self, num_users, duration_seconds):
        ...
        print(
            f"🚀 Starting load test: {num_users} users for {duration_seconds} seconds"
        )
    ...
    def _mixed_actions(self, user_id: int):
        ...
        actions = [
            (
                "volunteer_dashboard",
                lambda: self._api_call("GET", "/api/volunteer/dashboard"),
            ),
...
def run_scenario(scenario_name: str):
    ...
    scenarios = {
        ...
        "stress": {
            "users": 100,
            "duration": 180,
            "desc": "Stress test (100 users, 3min)",
        },
    }
...
def main():
    ...
    if len(sys.argv) == 2:
        ...
        try:
            ...
            print(
                "❌ Invalid arguments. Use: python load_test.py <users> <duration_seconds>"
            )
    else:
