...
def admin_login():
    ...
    if request.method == "POST":
        ...
        # Check credentials
        if admin_user and username == admin_user.username and admin_user.check_password(password):
            ...
            logger.warning(f"Failed login attempt for user: {username}, IP: {request.remote_addr}")
...
def admin_dashboard():
    ...
    try:
        ...
        pending_requests = HelpRequest.query.filter(HelpRequest.status == "pending").count()
        completed_requests = HelpRequest.query.filter(HelpRequest.status == "completed").count()
...
""",
            ...
            try:
                ...
                app.logger.error(f"Failed to save email to file: {file_e}")
                return jsonify({"success": False, "message": "Грешка при изпращане на имейл."}), 500

        return jsonify({"success": True, "message": "Нов код е изпратен на вашия имейл."})

    except Exception as e:
        app.logger.error(f"Error resending volunteer code: {e}")
        return jsonify({"success": False, "message": "Възникна грешка при изпращане на кода."}), 500
...
def volunteer_dashboard():
    try:
        ...
        try:
            ...
            # Reviews count
            reviews_count = TaskPerformance.query.filter_by(volunteer_id=volunteer_id).count()
        ...
        try:
            urgent_tasks_count = Task.query.filter_by(status="open", priority="urgent").count()
        except Exception as e:
        ...
        gamification = {
            ...
            "next_level_exp": (volunteer.level * 100)
            if hasattr(volunteer, "level")
            else 100,  # Simple calculation
        }
...
def update_volunteer_profile():
    ...
    try:
        ...
        return jsonify({"success": False, "message": "Грешка при обновяване на профила"}), 500
...
def update_volunteer_settings():
    ...
    try:
        ...
        return jsonify({"success": False, "message": "Грешка при запазване на настройките"}), 500
...
def api_get_chat_rooms():
    ...
    try:
        ...
        for room in rooms:
            ...
            online_count = ChatParticipant.query.filter_by(room_id=room.id, is_online=True).count()
...
def handle_send_chat_message(data):
    ...
    try:
        with app.app_context():
            ...
            logger.info(f"Message sent in room {room_id} by {sender_name} ({sender_type})")
...
def handle_file_upload(data):
    ...
    try:
        ...
        # Decode base64 file
        file_content = base64.b64decode(file_data.split(",")[1])  # Remove data:image/... prefix
        ...
        file_path = os.path.join(app.config["UPLOAD_FOLDER"], "chat_files", unique_filename)
...
def api_create_task():
    ...
    try:
        ...
        task = Task(
            ...
            created_by=session.get("admin_logged_in"),  # Use session instead of current_user
        )
        ...
        return jsonify(
            {
                ...
                "task": {
                    ...
                    "preferred_skills": json.loads(task.preferred_skills)
    ...
    try:
        ...
        if "preferred_skills" in data:
            task.preferred_skills = (
                json.dumps(data["preferred_skills"]) if data["preferred_skills"] else None
            )
...
def api_assign_task(task_id, volunteer_id):
    ...
    try:
        ...
        db.session.rollback()
        app.logger.error(f"Error assigning task {task_id} to volunteer {volunteer_id}: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
...
def api_volunteer_task_recommendations(volunteer_id):
    ...
    try:
        ...
        app.logger.error(f"Error getting task recommendations for volunteer {volunteer_id}: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
    ...
    try:
        ...
        if data_type == "dashboard":
            ...
            return jsonify({"error": "Infrom routes.notifications import notification_bp
