...
from models import (
    db, AnalyticsEvent, UserBehavior, PerformanceMetrics, 
    ChatbotConversation, Volunteer, Feedback
)
import logging
...
class AdvancedAnalytics:
    ...
    def track_event(self, event_type: str, event_category: str = None, 
                   event_action: str = None, event_label: str = None, 
                   event_value: int = None, context: Dict[str, Any] = None):
        """Проследява събитие за аналитика с thread safety"""
        return self._safe_database_operation(
            self._track_event_impl, 
            event_type, event_category, event_action, event_label, event_value, context
        )
    
    def _track_event_impl(self, event_type: str, event_category: str = None, 
                         event_action: str = None, event_label: str = None, 
                         event_value: int = None, context: Dict[str, Any] = None):
    ...
    def _track_performance_impl(self, metric_type: str, metric_name: str, 
                               metric_value: float, unit: str = None, 
                               context: Dict[str, Any] = None):
    ...
    def _get_overview_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        avg_session_time = db.session.query(
            func.avg(UserBehavior.total_time_spent)
        ).filter(
            UserBehavior.session_start.between(start_date, end_date)
        ).scalar() or 0
        ...
        bounced_sessions = db.session.query(
            func.count(UserBehavior.id)
        ).filter(
            and_(
                UserBehavior.bounce_rate == True,
                UserBehavior.session_start.between(start_date, end_date)
            )
        ).scalar() or 0
    ...
    def _get_user_engagement(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        device_stats = db.session.query(
            ...
            UserBehavior.session_start.between(start_date, end_date)
        ).group_by(UserBehavior.device_info).all()
        ...
        return {
            ...
            'device_breakdown': [{'device': device or 'Unknown', 'sessions': sessions} for device, sessions in device_stats],
    ...
    def _get_chatbot_analytics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        response_types = db.session.query(
            ...
            ChatbotConversation.created_at.between(start_date, end_date)
        ).group_by(ChatbotConversation.response_type).all()
        ...
        ai_stats = {
            ...
            'avg_confidence': sum(c.ai_confidence or 0 for c in ai_conversations) / len(ai_conversations) if ai_conversations else 0,
            'avg_processing_time': sum(c.processing_time or 0 for c in ai_conversations) / len(ai_conversations) if ai_conversations else 0,
        ...
        avg_rating = sum(c.user_rating for c in rated_conversations) / len(rated_conversations) if rated_conversations else 0
    ...
    def _get_performance_metrics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        ).group_by(PerformanceMetrics.endpoint).order_by(
            func.avg(PerformanceMetrics.metric_value).desc()
        ).limit(10).all()
        ...
        while current_date <= end_date:
            ...
            avg_response = db.session.query(
                func.avg(PerformanceMetrics.metric_value)
            ).filter(
                and_(
                    PerformanceMetrics.metric_type == 'response_time',
        ...
        return {
            'endpoint_performance': [{'endpoint': ep, 'avg_time': round(time, 3)} for ep, time in response_times],
    ...
    def _get_conversion_funnel(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        visited_register = db.session.query(
        ...
        started_registration = db.session.query(
            func.count(func.distinct(AnalyticsEvent.user_session))
        ).filter(
            and_(
        ...
        completed_registration = db.session.query(
            func.count(UserBehavior.id)
        ).filter(
            and_(
        ...
        # Използвали чатбота
        chatbot_users = db.session.query(
        ...
        return {
            ...
            'conversion_rates': {
                'visit_to_register_page': round(visited_register / total_visitors * 100, 2) if total_visitors else 0,
                'register_page_to_start': round(started_registration / visited_register * 100, 2) if visited_register else 0,
                'start_to_complete': round(completed_registration / started_registration * 100, 2) if started_registration else 0,
                'overall_conversion': round(completed_registration / total_visitors * 100, 2) if total_visitors else 0
            }
        }
    ...
    def _get_user_journey_analytics(self, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
        ...
        entry_pages = db.session.query(
            UserBehavior.entry_page,
            func.count(UserBehavior.id).label('entries')
        ).filter(
            and_(
                UserBehavior.session_start.between(start_date, end_date),
                UserBehavior.entry_page.isnot(None)
            )
        ).group_by(UserBehavior.entry_page).order_by(
            func.count(UserBehavior.id).desc()
        ).limit(10).all()
        
        # Най-чести exit points
        exit_pages = db.session.query(
            UserBehavior.exit_page,
            func.count(UserBehavior.id).label('exits')
        ).filter(
            and_(
                UserBehavior.session_start.between(start_date, end_date),
                UserBehavior.exit_page.isnot(None)
            )
        ).group_by(UserBehavior.exit_page).order_by(
            func.count(UserBehavior.id).desc()
        ).limit(10).all()
        ...
        common_paths = [{'path': path, 'count': count} for path, count in path_counter.most_common(10)]
        
        return {
            'top_entry_pages': [{'page': page, 'entries': entries} for page, entries in entry_pages],
    ...
    def _get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        active_users = db.session.query(
            ...
            UserBehavior.last_activity >= thirty_min_ago
        ).scalar() or 0
