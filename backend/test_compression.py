...
def test_compression():
    ...
    try:
        ...
        response2 = requests.get(
            f"{base_url}/analytics/stream", headers=headers, timeout=10
        )
        ...
        if encoding in ["gzip", "deflate", "br"]:
            ...
            if size2 < size1:
                ...
                print(
                    "   📊 Response size similar (may be small response or already optimized)"
                )
        else:
        ...
        print(
            f'   Content-Encoding: {response2.headers.get("Content-Encoding", "not set")}'
        )
        print(
            f'   Content-Length: {response2.headers.get("Content-Length", "not set")}'
        )
