...
def create_app(config_object=None) -> Flask:
    ...
    # --- Proposal Engine V1 route ---
    @app.get("/admin/proposal/<int:lead_id>")
    def proposal_engine_v1(lead_id):
        from datetime import datetime
        ...
        if "professional_leads" in tables:
            lead_table = "professional_leads"
        else:
            for table in tables:
                low = table.lower()
                if "lead" in low and "activities" not in low:
                    lead_table = table
                    break
            if not lead_table:
                for table in tables:
                    low = table.lower()
                    if "access_request" in low:
                        lead_table = table
                        break
        ...
        row = db.session.execute(
            sa_text(f"SELECT * FROM {lead_table} WHERE id = :id"),
            {"id": lead_id},
        ).mappings().first()
        ...
        org = pick("organization", "organisation", "organization_name", "company", "structure_name", default="Structure partenaire")
        contact = pick("contact_name", "name", "full_name", "profession", "role", default="Contact professionnel")
        email = pick("email", "contact_email", default="EUR")
        city = pick("city", "ville", default="France")
        status = pick("status", "stage", default="pilot")
        notes = pick("notes", "message", default="Besoin identifie : structurer le suivi, la qualification et la coordination operationnelle.")
        value = pick("estimated_value", "value", "amount", "mrr", default="490")
...
<style>
body{font-family:Arial,sans-serif;background:#f4f7fb;color:#10233f;margin:0;padding:32px}
.page{max-width:880px;margin:0 auto;background:white;border:1px solid #d8e2ef;border-radius:18px;padding:42px;box-shadow:0 20px 60px rgba(15,35,65,.12)}
.top{display:flex;justify-content:space-between;border-bottom:2px solid #e5edf7;padding-bottom:24px;margin-bottom:28px}
.brandline{display:flex;align-items:center;gap:12px;margin-bottom:14px}
.brandline img{width:46px;height:46px}
.brandname{font-size:20px;font-weight:900;color:#163a63}
.brandtag{font-size:12px;text-transform:uppercase;letter-spacing:.12em;color:#4f6f96;font-weight:800}
h1{font-size:34px;margin:8px 0 0;line-height:1.08}
...
button,a{display:inline-flex;align-items:center;justify-content:center;border:1px solid #cbd8ea;background:white;color:#12365f;padding:12px 16px;border-radius:12px;font-weight:800;text-decoration:none;cursor:pointer}
.primary{background:#245bd8;border-color:#245bd8;color:white}
@media print{body{background:white;padding:0}.page{box-shadow:none;border:none;border-radius:0}.actions{display:none}}
</style>
</head>
<body>
<div class="page">
  <div class="top">
    <div>
      ...
      <h1>Proposition de deploiement pilote</h1>
  ...
  <h2>Contexte identifie</h2>
  <p>{{ notes }}</p>

  <h2>Objectif du pilote</h2>
  <p>Centraliser les demandes, qualifier les situations, suivre les actions, securiser les acces et piloter les indicateurs cles.</p>

  <h2>Perimetre inclus</h2>
  <ul>
    <li>Entree structuree des demandes et qualification initiale.</li>
    <li>Suivi operationnel des situations, statuts et priorites.</li>
    <li>Tableau de bord de pilotage pour les equipes habilitees.</li>
    <li>Tracabilite des actions sensibles et connexions administratives.</li>
    <li>Accompagnement au cadrage et la prise en main.</li>
  </ul>

  <h2>Conditions proposees</h2>
  <div class="grid">
    <div class="card"><div class="label">DurEURe pilote</div><div class="value">60 jours</div></div>
    <div class="card"><div class="label">Tarif mensuel pilote</div><div class="price">{{ value }} EUR</div></div>
    <div class="card"><div class="label">Frais de mise en route</div><div class="value">Offerts pendant la phase pilote</div></div>
    <div class="card"><div class="label">etape actuelle</div><div class="value">{{ status }}</div></div>
  </div>

  <div class="cta">
    <h2>Prochaine etape proposee</h2>
    <p>Valider le perimetre du pilote, identifier un referent operationnel et planifier une reunion de lancement de 30 minutes.</p>
  </div>
</div>

<div class="actions">
  <a href="/admin/revenue">Retour Revenue</a>
  <button type="button" data-proposal-print class="primary">Imprimer / Enregistrer PDF</button>
</div>
<script defer src="/static/js/proposal-print.js"></script>
</body>
</html>
""", org=org, contact=contact, email=email, city=city, status=status, notes=notes, value=value, today=datetime.now().strftime("%d/%m/%Y"))



    # --- Proposal Engine Clean V2 ---
    @app.get("/admin/proposal-clean/<int:lead_id>")
    def proposal_engine_clean_v2(lead_id):
        from datetime import datetime
        from flask import render_template_string, abort
        from sqlalchemy import inspect, text as sa_text
        from backend.extensions import db
        ...
        inspector = inspect(db.engine)
        tables = inspector.get_table_names()
        lead_table = "professional_leads" if "professional_leads" in tables else None

        if not lead_table:
            abort(404)

        row = db.session.execute(
            sa_text("SELECT * FROM professional_leads WHERE id = :id"),
            {"id": lead_id},
        ).mappings().first()
        ...
        org = pick("organization", default="Structure partenaire")
        contact = pick("full_name", "profession", default="Contact professionnel")
        email = pick("email", default="EUR")
        city = pick("city", default="France")
        status = pick("status", default="pilote")
        value = pick("estimated_value", "value", "amount", "mrr", default="490")
        notes = pick("notes", "message", default="Besoin identifie : structurer le suivi, la qualification et la coordination operationnelle.")
        notes = notes.replace("Demo Paris Elite EUR", "Demo Paris Elite EUR")
