...
from models import (
    db, SupportedLanguage, TranslationKey, Translation, 
    UserLanguagePreference, ContentTranslation, TranslationMemory
)
...
class TranslationService:
    ...
    def get_supported_languages(self) -> List[SupportedLanguage]:
        ...
        return SupportedLanguage.query.filter_by(is_active=True).order_by(
            SupportedLanguage.is_default.desc(), SupportedLanguage.name
        ).all()
    ...
    def register_translation_key(self, key: str, source_text: str, 
                                category: str = 'general', context: str = None,
                                description: str = None, requires_html: bool = False,
                                max_length: int = None) -> TranslationKey:
    ...
    def get_translation(self, key: str, language_code: str = None, 
                       variables: Dict[str, Any] = None, 
                       fallback_to_source: bool = True) -> str:
        ...
        with self.cache_lock:
            ...
            if cached and cached['timestamp'] > datetime.utcnow() - timedelta(seconds=self.cache_timeout):
                text = cached['text']
            else:
    ...
    def _fetch_translation_from_db(self, key: str, language_code: str, 
                                  fallback_to_source: bool = True) -> str:
    ...
    def add_translation(self, key: str, language_code: str, translated_text: str,
                       translator_name: str = None, status: str = 'draft',
                       translation_method: str = 'manual') -> Optional[Translation]:
        """Добавя нов превод"""
        try:
            # Намираме ключа
            translation_key = TranslationKey.query.filter_by(key=key).first()
    ...
    def auto_translate_missing(self, target_language_code: str, 
                              source_language_code: str = None,
                              max_translations: int = 100) -> int:
        ...
        try:
            ...
            missing_keys = db.session.query(TranslationKey)\
    ...
    def _translate_with_ai(self, text: str, target_language: str, 
                          source_language: str = 'bg') -> Optional[Dict[str, Any]]:
        """Превежда текст с AI"""
        if not AI_TRANSLATION_AVAILABLE:
            return None
    ...
    def translate_content(self, content_type: str, content_id: int, 
                         language_code: str, title: str = None, 
                         content: str = None, **kwargs) -> Optional[ContentTranslation]:
    ...
    def get_content_translation(self, content_type: str, content_id: int, 
                               language_code: str) -> Optional[ContentTranslation]:
