...
extract_cmd = [
    sys.executable, "-m", "babel", "extract",
...
for lang in LANGUAGES:
    ...
    if not os.path.exists(po_file):
        ...
        subprocess.run([
            sys.executable, "-m", "babel", "init",
            "-i", "messages.pot", "-d", "translations", "-l", lang
        ], check=True)
    else:
        print(f"[2] Актуализиране на съществуващ превод за: {lang}")
        subprocess.run([
            sys.executable, "-m", "babel", "update",
            "-i", "messages.pot", "-d", "translations"
        ], check=True)
