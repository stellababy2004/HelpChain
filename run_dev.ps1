...
Copy-Item .\instance\hc_dev.db .\instance\hc_local_dev.db -Force
...
# Copy-Item .\instance\hc_dev.db "$env:TEMP\hc_local_dev.db" -Force
# $env:SQLALCHEMY_DATABASE_URI = "sqlite:///$env:TEMP/hc_local_dev.db"
# $env:HC_DB_PATH = "$env:TEMP/hc_local_dev.db"
# $env:DATABASE_URL = $env:SQLALCHEMY_DATABASE_URI
# & .\.venv\Scripts\python.exe -m flask --app backend.helpchain_backend.src.app:create_app run --no-reload --port 5005
