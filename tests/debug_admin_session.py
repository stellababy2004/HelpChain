...
with app.test_client() as client:
    ...
    def try_get(path, method='get', data=None):
        ...
        print('Headers:', {k: v for k, v in resp.headers.items() if k in ('Set-Cookie','Location')})
        try:
    ...
    print('POST headers:', {k: v for k, v in resp.headers.items() if k in ('Set-Cookie','Location')})
    try:
        print('POST body snippet:', resp.get_data(as_text=True)[:400].replace('\n',' '))
    except Exception:
        pass
    ...
    print('GET /admin/ headers:', {k: v for k, v in resp.headers.items() if k in ('Set-Cookie','Location')})
    try:
        print('GET /admin/ body snippet:', resp.get_data(as_text=True)[:400].replace('\n',' '))
    except Exception:
        pass
