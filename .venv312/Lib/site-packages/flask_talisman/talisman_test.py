...
class TestTalismanExtension(unittest.TestCase):
    ...
    def testFrameOptions(self):
        ...
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertEqual(response.headers["X-Frame-Options"], "ALLOW-FROM example.com")
    ...
    def testContentSecurityPolicyOptions(self):
        ...
        self.talisman.content_security_policy["image-src"] = ["'self'", "example.com"]
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        csp = response.headers["Content-Security-Policy"]
        self.assertIn("default-src 'self'", csp)
        ...
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertEqual(
            response.headers["Content-Security-Policy"], "default-src 'foo' spam.eggs"
        )
        ...
        self.assertIn(
            "default-src 'foo' spam.eggs", response.headers["Content-Security-Policy"]
        )
    ...
    def testContentSecurityPolicyOptionsReport(self):
        ...
        self.talisman.content_security_policy_report_uri = "https://example.com"
        response = self.client.get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertIn("Content-Security-Policy-Report-Only", response.headers)
        self.assertIn(
            "report-uri", response.headers["Content-Security-Policy-Report-Only"]
        )
        self.assertNotIn("Content-Security-Policy", response.headers)
        ...
        self.assertIn("Content-Security-Policy-Report-Only", response.headers)
        self.assertIn(
            override_report_uri, response.headers["Content-Security-Policy-Report-Only"]
        )

        # exception on missing report-uri when report-only
        self.assertRaises(
            ValueError, Talisman, self.app, content_security_policy_report_only=True
        )
    ...
    def testContentSecurityPolicyNonce(self):
        ...
        with self.app.test_client() as client:
            ...
            self.assertIn(
                "script-src 'self' 'nonce-{}'".format(flask.request.csp_nonce), csp
            )
            self.assertNotIn("style-src 'self'", csp)
            self.assertNotIn(
                "style-src example.com 'nonce-{}'".format(flask.request.csp_nonce), csp
            )
            self.assertIn("style-src example.com", csp)
            self.assertIn(flask.request.csp_nonce, response.data.decode("utf-8"))
    ...
    def testBadEndpoint(self):
        ...
        self.assertEqual(response.status_code, 302)
        response = self.client.get(
            "/bad_endpoint", headers={"X-Forwarded-Proto": "https"}
        )
    ...
    def testPermissionsPolicy(self):
        ...
        response = app.test_client().get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertIn(
            "vibrate=(), geolocation=()", response.headers["Permissions-Policy"]
        )
    ...
    def testDocumentPolicy(self):
        ...
        response = app.test_client().get("/", environ_overrides=HTTPS_ENVIRON)
        self.assertIn(
            "oversized-images=?0, document-write=?0",
