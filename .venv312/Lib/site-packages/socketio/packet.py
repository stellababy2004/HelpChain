...
class Packet:
    ...
    def __init__(
        ...
        if self.uses_binary_events and (
            binary or (binary is None and self.data_is_binary(self.data))
        ):
    ...
    def _reconstruct_binary_internal(cls, data, attachments):
        if isinstance(data, list):
            return [
                cls._reconstruct_binary_internal(item, attachments) for item in data
            ]
            ...
            if data.get("_placeholder") and "num" in data:
                ...
                return {
                    key: cls._reconstruct_binary_internal(value, attachments)
    ...
    def _deconstruct_binary_internal(cls, data, attachments):
        if isinstance(data, (bytes, bytearray)):
            ...
            return [
                cls._deconstruct_binary_internal(item, attachments) for item in data
            ]
        elif isinstance(data, dict):
            return {
                key: cls._deconstruct_binary_internal(value, attachments)
                for key, value in data.items()
            }
        else:
            return data
    ...
    def data_is_binary(cls, data):
        ...
        if isinstance(data, (bytes, bytearray)):
            ...
            return functools.reduce(
                lambda a, b: a or b, [cls.data_is_binary(item) for item in data], False
            )
        elif isinstance(data, dict):
            return functools.reduce(
                lambda a, b: a or b,
                [cls.data_is_binary(item) for item in data.values()],
                False,
            )
        else:
            return False
