...
try:
    user_mod_path = os.path.join(_here, "backend/appy.py")  # absolute
    __vc_spec = util.spec_from_file_location("backend.appy", user_mod_path)
    __vc_module = util.module_from_spec(__vc_spec)
    sys.modules["backend.appy"] = __vc_module
    __vc_spec.loader.exec_module(__vc_module)
    __vc_variables = dir(__vc_module)
except Exception:
    _stderr("Error importing backend/appy.py:")
...
if "VERCEL_IPC_PATH" in os.environ:
    ...
    if "handler" in __vc_variables or "Handler" in __vc_variables:
        ...
        if not inspect.iscoroutinefunction(
            ...
            try:
                ...
                _stderr(
                    "Uvicorn is required to run ASGI apps. Please ensure it is installed."
                )
                exit(1)
    ...
    _stderr('Missing variable `handler` or `app` in file "backend/appy.py".')
    _stderr(
        "See the docs: https://vercel.com/docs/functions/serverless-functions/runtimes/python"
    )
    exit(1)
...
if "handler" in __vc_variables or "Handler" in __vc_variables:
    ...
    print('Missing variable `handler` or `app` in file "backend/appy.py".')
    print(
        "See the docs: https://vercel.com/docs/functions/serverless-functions/runtimes/python"
    )
    exit(1)
